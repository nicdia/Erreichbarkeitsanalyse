    ...
    try:
        with db_con.connect() as conn:
            ...
            for table, table_data in fetch_results.items():
                file_name = (
                    params["mode"].lower().replace(",", "") + "_iso_" +
                    str(params["speed"]).replace(".", "_").replace(",", "") + "km_" +
                    table.lower().replace(".", "_").replace(",", "") + ".geojson"
                )
...
################################################################################
with open ("C:\\Master\\GeoinfoPrj_Sem1\\Erreichbarkeitsanalyse\\fetch_data\\transit_student\\fetch_transit_config.json", "r") as file:
